...
export async function getServerSideProps(context){
  ...
  try{
    ...
    // Determine selected path from query (default to Beginner) so we can filter server-side
    ...
    const filteredRows = (rows || []).filter(r => {
      // Beginner: exclude any articles tagged Intermediate or Advanced
      ...
      // If Intermediate selected, include ONLY articles that have Intermediate tag
      if (selectedLabel === 'Intermediate') {
        return tagIncludes(r.tags || r.tag || r.tags_list || r.labels || r.tagsString, 'Intermediate');
      }
      // If Advanced selected, include ONLY articles that have Advanced tag
