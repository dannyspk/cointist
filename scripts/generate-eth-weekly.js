...
async function generateNarrativeWithLLM(signals, priceSummary){
  ...
  try{
    if (String(model).toLowerCase().startsWith('gpt-5')){
      ...
      const doRequest = async (maxOutputTokens) => {
        ...
        if(!res || !res.ok){ const txt = res ? await res.text().catch(()=>'<no body>') : '<no response>'; console.warn('LLM narrative request failed', res && res.status, txt); try{ if(process.env.OPENAI_DEBUG === 'true'){ console.warn('Model used:', model); console.warn('Request body snippet:', JSON.stringify(body).slice(0,2000)); console.warn('Full response body (truncated 8000 chars):', txt && String(txt).slice(0,8000)); } }catch(_){ } return null; }
      ...
      if (!json){
        ...
        try{
          ...
          const chatBody = { model: chatModel, messages:[{role:'system', content:'You are a professional financial market analyst. Be concise and factual.'},{role:'user', content:prompt}], max_tokens:400, temperature:0.0 };
          const chatRes = await (fetchFn ? fetchFn(url, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify(chatBody) }) : fetch(url, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify(chatBody) }));
  ...
  try{ return JSON.parse(jtxt); }catch(e){ console.warn('Failed to parse LLM JSON response; raw content follows:\n', jtxt); try{ console.warn('Full API JSON:', JSON.stringify(json).slice(0,2000)); }catch(_){} return null; }
    }else{
      const url = 'https://api.openai.com/v1/chat/completions';
      const body = { model, messages:[{role:'system', content:'You are a professional financial market analyst. Be concise and factual.'},{role:'user', content:prompt}], max_tokens:400, temperature:0.6 };
      const res = await (fetchFn ? fetchFn(url, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify(body) }) : fetch(url, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify(body) }));
      if(!res || !res.ok){ const txt = res ? await res.text().catch(()=>'<no body>') : '<no response>'; console.warn('LLM narrative request failed', res && res.status, txt); return null; }
