#!/usr/bin/env node
/**
 * Delete any existing row in 'Article' with slug 'wallets-101' then upsert the same content into 'Guides'.
 * Uses SUPABASE_SERVICE_ROLE_KEY from .vercel.env
 * Usage: node scripts/upsert-wallets-101-to-guides-supabase.js
 */
...
// Choose an Opinions author from existing content
const opinionsAuthor = 'Sushant Pawan';

const guide = {
  slug: 'wallets-101',
  title: 'Wallets 101: Custodial vs Non‑Custodial',
  category: 'Guides',
  author: opinionsAuthor,
...
async function main(){
  try{
    // Delete any existing entry in Article with this slug
    const del = await supa.from('Article').delete().eq('slug', guide.slug).select();
    if (del.error) {
      console.error('Error deleting existing Article row:', del.error.message || del.error);
    } else if (del.data && del.data.length) {
      console.log('Deleted Article rows:', del.data.map(d => d.id));
    } else {
      console.log('No Article rows to delete for slug', guide.slug);
    }

    // Upsert into Guides table
    const { data, error } = await supa.from('Guides').upsert([guide], { onConflict: 'slug' }).select();
    if (error) {
      console.error('Upsert to Guides failed:', error.message || error);
      process.exit(1);
    }
    console.log('Guides upsert result:', data && data[0] ? data[0] : data);
  }catch(e){
